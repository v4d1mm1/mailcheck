...
class CustomFileHandler(logging.Handler):
    ...
    def emit(self, record):
        try:
            ...
            obfuscated_msg = self.obfuscate_emails(
                msg) if self.obfuscate else msg
    ...
    def obfuscate_emails(self, msg):
        def replace_email(match):
            ...
            obfuscated_mailbox = mailbox[:len(
                mailbox) // 2] + '*' * (len(mailbox) // 2)
            # No reason to obfuscate if domain is among top domains
            if domain not in self.TOP_DOMAINS:
                corp_domain, top_level_domain = domain.split('.')
                obfuscated_domain = '*' * \
                    (len(corp_domain) // 2) + \
                    corp_domain[len(corp_domain) // 2:] + \
                    '.' + top_level_domain
            else:
                obfuscated_domain = domain
            return obfuscated_mailbox + '@' + obfuscated_domain

        # Replace emails with obfuscated
